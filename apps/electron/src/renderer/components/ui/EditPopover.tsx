...
export interface EditPopoverProps {
  ...
  /** Optional explicit model id or tier hint like 'fast'/'default'. */
  model?: CreateSessionOptions['model']
  /** Optional LLM connection slug for created sessions. */
  llmConnection?: CreateSessionOptions['llmConnection']
  /** Optional initial thinking level for inline sessions. */
  thinkingLevel?: ThinkingLevel
  /** Optional enabled sources for the created session. */
...
export function EditPopover({
  ...
  const handleInlineSendMessage = useCallback(async (message: string) => {
    const builder = promptBuilder ?? ((input: string) => buildEditPrompt(context, input, displayLabel))
  ...
  const handleLegacySendMessage = useCallback((message: string) => {
    ...
    setOpen(false)
  }, [context, displayLabel, promptBuilder, workingDirectory, currentModel, currentConnection, systemPromptPreset, permissionMode, setOpen])
